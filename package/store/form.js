...
export default {
  ...
  mutations: {
    ...
    DELETE_FIELD: (state, payload) => {
      ...
      // 先看根级元素
      const deleteIndex = state.value.fields.findIndex(item => item.uuid === payload.uuid)
      ...
      if (deleteIndex >= 0) {
        ...
        eachTreeNodes(state.value.fields, node => {
          ...
          const deleteIndex = node.children.findIndex(item => item.uuid === payload.uuid)
          if (deleteIndex < 0) {
            return true
          }
    ...
    UPDATE_EDITING: (state, payload) => {
      state.fieldMap[payload.uuid] = Object.assign(state.fieldMap[payload.uuid], payload)
    ...
    SET_LISTENERS: (state, payload) => {
      const { value } = payload
      state.value.listeners = [...state.value.listeners, { actions: [], ...value }]
    },
